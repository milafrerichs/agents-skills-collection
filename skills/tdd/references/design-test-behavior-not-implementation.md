...
```typescript
describe('sortUsers', () => {
  it('calls quicksort with correct comparator', () => {
    const quicksortSpy = vi.spyOn(sortUtils, 'quicksort')

    sortUsers(users, 'name')

    expect(quicksortSpy).toHaveBeenCalledWith(users, expect.any(Function))
    // Breaks if we switch to mergesort, even though behavior is identical
  })
})

describe('UserService', () => {
  it('caches user in internal Map', () => {
